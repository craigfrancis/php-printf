--TEST--
Test vprintf() function : basic functionality - hexadecimal format
--FILE--
<?php
echo "*** Testing vprintf() : basic functionality - using hexadecimal format ***\n";

// Initialising different format strings
$format = "format";
$format1 = "%x";
$format2 = "%x %x";
$format3 = "%x %x %x";

$format11 = "%X";
$format22 = "%X %X";
$format33 = "%X %X %X";

$arg1 = array(11);
$arg2 = array(11,132);
$arg3 = array(11,132,177);

$result = user_vprintf($format1,$arg1);
echo "\n";
var_dump($result);
$result = user_vprintf($format11,$arg1);
echo "\n";
var_dump($result);

$result = user_vprintf($format2,$arg2);
echo "\n";
var_dump($result);
$result = user_vprintf($format22,$arg2);
echo "\n";
var_dump($result);

$result = user_vprintf($format3,$arg3);echo "\n";
var_dump($result);
$result = user_vprintf($format33,$arg3);
echo "\n";
var_dump($result);

?>
--EXPECT--
*** Testing vprintf() : basic functionality - using hexadecimal format ***
b
int(1)
B
int(1)
b 84
int(4)
B 84
int(4)
b 84 b1
int(7)
B 84 B1
int(7)
