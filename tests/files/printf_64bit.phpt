...
$name = "voudras";
try {
    user_printf("%d $string %s", $nbr, $name);
...
foreach( $float_variation as $float_var )
{
 ...
 {
  echo "\n";
  user_printf( $float_var, $float_num );
 }
 $counter++;
}
...
user_printf("%%%.2f",1.23456789e10);

echo"\n\n*** Output for '%%' as the format parameter ***\n";
user_printf("%%",1.23456789e10);

echo"\n\n*** Output for precision value more than maximum ***\n";
user_printf("%.988f",1.23456789e10);

echo"\n\n*** Output for invalid width(-15) specifier ***\n";
try {
    user_printf("%030.-15s", $tempstring);
} catch (ValueError $e) {
    echo $e->getMessage();
}

echo"\n\n*** Output for '%F' as the format parameter ***\n";
user_printf("%F",1.23456789e10);
